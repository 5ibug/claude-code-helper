...
const CLAUDE_ENV_VARS = [
  {
    ...
    descriptionKey: 'home.envVarDescription',
...
export default function HomePage() {
  ...
  const loadEnvVars = async () => {
    ...
    try {
      ...
      const mergedVars = CLAUDE_ENV_VARS.map(claudeVar => {
        ...
        return {
          ...claudeVar,
          description: t(claudeVar.descriptionKey),
  ...
  const installTool = async (tool) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(`${t('home.installing')}: ${data.message || data.error}`);
      }
    } catch (error) {
      alert(`${t('home.installing')}: ${error.message}`);
    }
    setInstalling(null);
  };
  ...
  const handleSaveEnvVar = async (name, value) => {
    if (!value.trim()) {
      alert(t('envVars.valueRequired'));
      return;
    }
    try {
      ...
      if (data.success) {
        ...
        alert(`${t('envVars.saveFailed')}: ${data.message || data.error}`);
      }
    } catch (error) {
      alert(`${t('envVars.saveFailed')}: ${error.message}`);
    }
  };
  ...
  return (
    <div className="page-container">
      ...
      {loading ? (
        <div className="loading-state">{t('home.checking')}</div>
      ) : (
            ...
            return (
              <div key={tool} className={`env-card ${info.installed ? 'installed' : 'not-installed'}`}>
                ...
                <div className="card-content">
                  ...
                  {info.installed ? (
                    ...
                    <div className="card-status not-installed">
                      <span className="status-badge">{t('common.notInstalled')}</span>
                    </div>
                  )}
                </div>
                {!info.installed && installableTools.includes(tool) && (
                  ...
                  >
                    {installing === tool ? t('home.installing') : t('home.oneClickInstall')}
                  </button>
                )}
              </div>
            );
              ...
              <div className="card-header">
                <div className="var-name-section">
                  ...
                  <span className={`status-badge ${envVar.isSet ? 'set' : 'unset'}`}>
                    {envVar.isSet ? t('common.set') : t('common.notSet')}
                  </span>
                </div>
                ...
                {editingVar === envVar.name ? (
                  <div className="edit-section">
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') {
                          handleSaveEnvVar(envVar.name, e.target.value);
                    ...
                    <div className="edit-actions">
                      <button
                        ...
                        onClick={(e) => {
                          ...
                          handleSaveEnvVar(envVar.name, input.value);
                        }}
                      >
                        {t('common.save')}
                      </button>
                  ...
                  <div className="view-section">
                    <div className="value-display">
                      {envVar.value ? (
                        ...
                        <span className="value-placeholder">{t('common.example')}: {envVar.example}</span>
                      )}
                    </div>
                    <button
                    ...
                    >
                      {envVar.isSet ? t('common.edit') : t('envVars.setVar')}
                    </button>
                  </div>
                )}
              </div>
